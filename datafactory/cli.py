...
def build(
    ...
    # Stage 1: Resolve City via bulk GeoNames adapter
    city_resolved_path = settings.raw_dir / country_slug / state_slug / city_slug / "city_resolved.json"
    geonames = GeoNamesBulkSource()
    city_meta = geonames.resolve_city(city_name=city, state_name=state, country_name=country)

    if not city_meta and city_resolved_path.exists():
        console.print(f"[Stage 1] Loading existing city resolution from {city_resolved_path}")
        with open(city_resolved_path, "r", encoding="utf-8") as f:
            c_dict = json.load(f)
            city_meta = CityMetadata(**c_dict)

    if not city_meta:
        raise ValueError(f"Unable to resolve canonical city entity for {city}, {state}, {country}")
