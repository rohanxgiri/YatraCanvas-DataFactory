...
class CanonicalPlaceGraph:
    ...
    def _check_entity_agreement(
        ...
        if dist is not None:
            ...
            # Guard against false merges of distinct neighboring businesses sharing only generic tokens (e.g. Restaurant, Hotel, Cafe)
            generic_merge_tokens = {
                "restaurant", "cafe", "coffee", "hotel", "resort", "inn", "guest", "house",
                "guesthouse", "hostel", "dhaba", "bhojanalaya", "bar", "club", "store", "shop",
                "temple", "mandir", "masjid", "mosque", "church", "gurudwara", "point", "center",
                "centre", "complex", "bazaar", "market", "emporium", "agency", "services",
            }
            tokens_b = {w for w in normalize_name(base_name).split() if len(w) > 2 and w not in generic_merge_tokens}
            tokens_c = {w for w in normalize_name(cand_name).split() if len(w) > 2 and w not in generic_merge_tokens}
            has_distinctive = bool(tokens_b and tokens_c)
            distinctive_overlap = bool(tokens_b & tokens_c)

            # Check stem overlap (e.g. mambo vs mambos)
            if not distinctive_overlap and has_distinctive:
                for tb in tokens_b:
                    for tc in tokens_c:
                        if tb.rstrip("s") == tc.rstrip("s") and len(tb.rstrip("s")) >= 4:
                            distinctive_overlap = True
                            break

            distinctive_sim = 0.0
            if has_distinctive:
                distinctive_sim = fuzzy_name_similarity(" ".join(sorted(tokens_b)), " ".join(sorted(tokens_c)))

            # If both have distinctive tokens, but zero overlap and low similarity:
            # They are distinct neighboring entities (e.g. Ganga Jamuna Restaurant vs Mamta Restaurant at 23m)
            if has_distinctive and not distinctive_overlap and distinctive_sim < 0.45:
                return False, None, 0.0, None

            if dist <= max_dist and max_sim >= 0.80 and (distinctive_overlap or not has_distinctive or distinctive_sim >= 0.60):
                return True, f"spatial_proximity_{int(dist)}m_name_sim_{round(max_sim, 2)}", round(max_sim, 2), None
            elif dist <= 25 and max_sim >= 0.70 and (distinctive_overlap or not has_distinctive or distinctive_sim >= 0.60):
