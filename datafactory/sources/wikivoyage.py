...
class WikivoyageSource:
    ...
    def fetch_city_listings(
        ...
        # Download article wikitext from Wikivoyage
        wikitext = self._fetch_article_wikitext(city_name)
        if not wikitext:
        ...
        print(f"[Wikivoyage] Extracted and cached {len(listings)} structured listings for {city_name}")
