...
class CityNotFoundInRequestedState(ValueError):
    """Raised when a city is not found within the strictly requested administrative state/country."""
    pass


def is_state_match(cand_state: str, requested_state: str) -> bool:
    """Case and alias-insensitive state name matching."""
    if not cand_state or not requested_state:
        return True
    s1 = normalize_name(cand_state)
    s2 = normalize_name(requested_state)
...
class GeoNamesBulkSource:
    ...
    def resolve_city(
        ...
        Enforces country and state as strict, mandatory filters.
        ...
        # HARD CONSTRAINT: Candidates must match country and state first
        state_candidates = []
        for c in cities:
            if target_cc and c["country_code"] != target_cc:
                continue
            if norm_state and state_name:
                if not is_state_match(c["state"], state_name):
                    continue
            state_candidates.append(c)

        if norm_state and not state_candidates:
            raise CityNotFoundInRequestedState(
                f"CITY_NOT_FOUND_IN_REQUESTED_STATE: No GeoNames records match requested state '{state_name}', country '{country_name}'"
            )
        ...
        if not candidates:
            # Fallback: fuzzy search if exact match not found within the requested state candidates
            for c in state_candidates:
        ...
        if not candidates:
            if norm_state:
                raise CityNotFoundInRequestedState(
                    f"CITY_NOT_FOUND_IN_REQUESTED_STATE: City '{city_name}' not found in state '{state_name}', country '{country_name}' in GeoNames"
                )
        ...
        return CityMetadata(
            ...
            resolution_source="geonames_cities15000",
