...
- **Architecture & pipeline boundaries** — does code in the right place own the right responsibilities? No data extraction or network calls inside pure transform functions. No business logic in exporters or UI templates. Are pipeline stages separated cleanly? Whatever the project's boundaries are — are they respected?
- **Domain contracts & data models** — do models conform strictly to schemas (e.g. Pydantic models, dataclasses, types)? Are validation rules, quarantine logic, and zero-hallucination policies enforced?
- **Design system & visual standards** (if UI/reporting) — are the correct CSS variables, tokens, classes, and templates used? Any hardcoded raw hex values or inconsistent styling?
- **Code standards & typing** — naming conventions, file organisation, type annotations (Python type hints / TypeScript), exception handling patterns — do they match what the project established?
- **Existing patterns** — does this change introduce an ad-hoc pattern when an established pattern or shared utility already exists?

### Layer 3 — Is it production ready?

Check:

- Error handling — what happens when things go wrong? Are network, API, parsing, and I/O failures handled cleanly with appropriate logging, or does it fail silently?
- Edge cases & data validity — empty inputs, malformed responses, null fields, out-of-boundary values — are these handled gracefully?
- Diagnostics & warnings — any warnings, deprecations, or unhandled exceptions in the terminal, test suites, or console?
- Tests & verification — are existing tests passing, and are new critical paths covered by unit/integration tests?
- Obvious bugs — anything that would corrupt state, leak resources, or break for an end user?
