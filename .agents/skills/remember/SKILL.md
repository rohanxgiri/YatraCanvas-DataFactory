...
**What was built** — specific files created or modified, features completed, components or pipeline modules added. Be precise. Not "built the auth flow" — "created app/(auth)/login/page.tsx and middleware.ts" or "implemented spatial grid deduplication in datafactory/pipeline/dedupe.py and added test cases in tests/test_dedupe.py".

**Decisions made** — choices that would be hard to reverse or that future work depends on. Not minor implementation details — architectural choices. "Chose spatial cell bucketing over pairwise coordinate distance to reduce deduplication complexity from O(N²) to O(N)."

**Problems solved** — any issue that took time to figure out. So the next session does not solve the same problem twice. "Nominatim rate limits exceeded during bulk lookups — resolved by introducing exponential backoff with jitter in geocoding client."
...
- `GEMINI.md`, `.agents/rules/*.md` — Antigravity / Gemini
- `AGENTS.md` — Codex / Antigravity / universal agent rules
